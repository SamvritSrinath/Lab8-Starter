  ...
  //            declared above
    for(const recipe_url of RECIPE_URLS){
      try {
        ...
        // console.log(`Fetching recipe from: ${recipe_url}`); debugging statement
        const response = await fetch(recipe_url);
        const recipe_json = await response.json();

        fetchedRecipes.push(recipe_json);
        //validation to see if all recipes have been fetched
